...
namespace MediaBazaarApp
{
    public class EmployeeProfile
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("UPDATE EmployeeData SET App_Color = '" + color + "' WHERE Employee_ID = '" + EmployeeID + "' AND Password = '" + Password + "'", cnn.sqlConection()))
            {
                cnn.Open();
                cmd.ExecuteNonQuery();
                cnn.Close();
